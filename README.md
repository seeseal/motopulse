...
A dark, full-featured riding companion app for motorcyclists. Built with Flutter — tracks rides with navigation-grade accuracy, connects groups in real time via Firebase, detects crashes, stores your documents, and handles emergencies.
...
- **ONGOING RIDE button** — the Start Ride button is reactive. When a ride is already active (even if you navigated away), it turns dark with a red glow and shows a live elapsed timer and current distance. Tap it to return to the tracking screen. When no ride is active, it shows a red Start Ride button
...
**GPS & accuracy**
- Location updated every second using Android's fused location provider at `bestForNavigation` accuracy
- GPS fixes with accuracy worse than 20 m are silently discarded — this eliminates the "phantom" jumps that make paths appear to cut through buildings
- Teleport filter rejects any single reading that moves more than 500 m from the last point (catches GPS glitches)
- Background GPS keeps running with the screen off — the foreground service holds a wake lock and requests battery optimisation exemption automatically on ride start
...
- **Heading-up camera** — the map rotates to keep your current direction of travel pointing upward, exactly like Google Maps navigation. Camera tilts to 45° for a perspective view of the road ahead
- **North-up toggle** — tap the blue compass button to switch between heading-up (navigation mode) and north-up (overview mode). Switching to north-up resets tilt to flat
...
- **Traffic layer toggle** — a traffic button on the map overlay shows or hides live traffic conditions (Google Maps traffic data). The button turns red when traffic is active
...
- Group Ride shortcut tile when not riding
...
**Crash detection**
- Accelerometer monitored continuously during an active ride
- A sudden high-G impact triggers an audible alert overlay with a countdown — if you don't dismiss it, SOS fires automatically
...
- Tap any ride to see a dark Google Map mini-map with the full route replayed as a red polyline
...
- Search any address using Nominatim (OpenStreetMap geocoding, no API key)
...
- Works even if you're the only one in the group — the QR code is always available offline
...
Keep a service log for your bike.
...
- **Bike details** — bike name, fuel tank capacity (litres), fuel efficiency (km/L). These drive the live range estimator on the tracking screen
- **Speed alert threshold** — set the km/h value that triggers the ride tracking speed alert
- **Live QR preview** — the emergency QR card updates in real time as you fill in your details, so you can see exactly what first responders will see
...
| Background GPS | `flutter_foreground_task` (foreground service + wake lock) |
| Crash detection | `sensors_plus` (accelerometer) |
...
- A Google Maps API key with **Maps SDK for Android** enabled in Google Cloud Console
...
**Important** — in Google Cloud Console, open the API key's restrictions and ensure **Maps SDK for Android** is listed under the allowed APIs. The Firebase-auto-created Android key does not include it by default.
...
- GPS accuracy filter — fixes worse than 20 m accuracy discarded to prevent path hallucination
